...
MDS_parameter_strct* MDS_parameter_reader(xmlNodePtr node, xmlDocPtr doc)
{
    ...
    mds_parameter_strct->value_list = (double*) get_double_vec(mds_parameter_strct->n_value);
    ...
    while (cur_node != NULL)
    {
        if (cur_node->type == XML_ELEMENT_NODE || cur_node->type == XML_COMMENT_NODE)
        {
            if(!strcmp((const char*)cur_node->name,"type"))
            {
                ...
                if(!strcmp((const char*)elementValue,"lut3D"))
                {
...
void MDS_structurer(MDS_gen_strct* mds_gen_strct)
{
    ...
    mds_gen_strct->sensor_point_list = get_int_vec(mds_gen_strct->n_sensor);
    mds_gen_strct->extforce_point_list = get_int_vec(mds_gen_strct->n_extforce);
...
MDS_base_strct* init_MDS_base_strct(void)
{
    ...
    mds_base_strct->gravity = get_double_vec(3);
...
void free_MDS_base_strct(MDS_base_strct *mds_base_strct)
{
    ...
    free_double_vec(mds_base_strct->gravity);
...
MDS_body_strct* init_MDS_body_strct(char* name)
{
    ...
    mds_body_strct->com = get_double_vec(3);
    mds_body_strct->inertia = get_double_vec(6);
...
void free_MDS_bodytree_strct(MDS_bodytree_strct *mds_bodytree_strct)
{
    ...
    free_int_vec(mds_bodytree_strct->qu);
    free_int_vec(mds_bodytree_strct->qv);
    free_int_vec(mds_bodytree_strct->qc);
    free_int_vec(mds_bodytree_strct->qlocked);
    free_int_vec(mds_bodytree_strct->qdriven);
    free_int_vec(mds_bodytree_strct->qa);
...
MDS_ball_strct* init_MDS_ball_strct(char* name)
{
    ...
    mds_ball_strct->ignore = get_int_vec(3);
...
void free_MDS_ball_strct(MDS_ball_strct *mds_ball_strct)
{
    ...
    free_int_vec(mds_ball_strct->ignore);
    free(mds_ball_strct);
}
...
MDS_solid_strct* init_MDS_solid_strct(char* name)
{
    ...
    mds_solid_strct->ignore = get_int_vec(3);
...
void free_MDS_solid_strct(MDS_solid_strct *mds_solid_strct)
{
    ...
    free_int_vec(mds_solid_strct->ignore);
...
void free_MDS_parameter_strct(MDS_parameter_strct *mds_parameter_strct)
{
    ...
    {
        free_double_vec(mds_parameter_strct->value_list);
    }
...
void free_MDS_gen_strct(MDS_gen_strct *mds_gen_strct)
{
    ...
    free_int_vec(mds_gen_strct->sensor_point_list);
    free_int_vec(mds_gen_strct->extforce_point_list);
