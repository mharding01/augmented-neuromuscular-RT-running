...
MbsBuffer* mbs_new_buffer(char* filename, char* anim_name, int nx, int size, int id, int save_anim, int save_visu, double anim_period){
    ...
    b->t = (double*)malloc(b->size*sizeof(double));
    
    // initialize value vector
    b->x = (double*)malloc(b->size*b->nx*sizeof(double));
...
void mbs_anim_write(MbsBuffer* b)
{
    ...
    while (cur_t <= b->t[b->index-1])
    {
        if ((prev_t <= cur_t) && (cur_t <= next_t))
        {
            ...
            {
                // write time
                print_t = b->t[next_index];
                fprintf(fid, "%g", print_t);
                
                // write array
                for (i=0; i<b->nx ; i++)
                {
                    fprintf(fid, "  %g", b->x[next_index*b->nx + i]);
                }
            ...
            else
            {
                ...
                for (i=0; i<b->nx ; i++)
                {
                    if (next_index > 0)
                    {
                        print_x = alpha_comp * b->x[(next_index-1)*b->nx + i] + alpha * b->x[next_index*b->nx + i];
                    }
                    else
                    {
                        print_x = alpha_comp * b->last_x[i] + alpha * b->x[next_index*b->nx + i];
                    }
