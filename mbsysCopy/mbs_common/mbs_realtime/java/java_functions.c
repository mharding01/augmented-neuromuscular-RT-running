...
#endif
{
    ...
    jmethodID getNbVpMethod, setVpMethod;
    ...
    mbsFilename = (*env)->NewStringUTF(env, jni_in_out->mbs_file);
    ...
    doubleArrayArg = (*env)->NewDoubleArray(env, jni_in_out->nb_q);

    // copy the current values of q_vec to the jni array
    (*env)->SetDoubleArrayRegion(env, doubleArrayArg, 0 , jni_in_out->nb_q, jni_in_out->q_vec);
    ...
    run_java_mac_os(jni_in_out->jni_struct, jni_in_out->nb_q);
...
void update_jni(JNI_struct *jni_struct, Realtime_java *java, int nb_q, double *q_vec)
{
    // variables declaration
        ...
        write(pipe_q_vec[1], q_vec, nb_q*sizeof(double));
    #else
        env = jni_struct->env;
 ...
 * \param[in] nb_q number of joints for the visualization
 */
void run_java_mac_os(JNI_struct *jni_struct, int nb_q)
{
    // variables declaration
    int flag_finish, flag_close, flag_viewpoint;
    double *q_vec;
    jdoubleArray doubleArrayArg;
    JNIEnv* env;
    ...
    // vector to get joint position
    q_vec = (double*) malloc(nb_q*sizeof(double));
    ...
    while (1)
    {
        ...
        read(pipe_q_vec[0], q_vec, nb_q*sizeof(double));
