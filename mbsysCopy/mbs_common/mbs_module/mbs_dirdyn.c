...
void mbs_dirdyn_init(MbsDirdyn* dd, MbsData* mbs_data)
{
    ...
    if(dd->options->save2file)
    {
        ...
        if (dd->options->save_visu)
        {
            ...
            sprintf(f,      "%s/visu_q.res",  respath);
            sprintf(f_anim, "%s/visu_q.anim", animpath);

            dd->buffer_visu = mbs_new_buffer(f, f_anim, java->nb_q, dd->options->buffersize, BUFFER_VISU, dd->options->save_anim, dd->options->save_visu, 1./(double)dd->options->framerate);
...
void mbs_dirdyn_finish(MbsDirdyn* dd, MbsData* mbs_data){
    ...
    if(dd->options->save2file)
    {
        ...
        if (dd->options->save_visu)
        {
            mbs_buffer_write(dd->buffer_visu);
            mbs_delete_buffer(dd->buffer_visu);
        }
