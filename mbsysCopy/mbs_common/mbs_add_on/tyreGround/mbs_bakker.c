...
// -------------------
    ...
    A0  = 1.65;
    A1  = -21.3*pow(FradkN,2)+1144*FradkN;
    A2  = (49.6*pow(FradkN,2)+226*FradkN)/(A0*A1*exp(0.069*FradkN));
...
// --------------
    ...
    B2  = (1078*sin(1.82*atan(0.208*FradkN))*(1-0.022*fabs(angcambdeg)))/(B0*B1);
    B3  = -0.354*FradkN+0.707;
    deltaSh = 0.028*angcambdeg;
    deltaSv = (14.8*FradkN)*angcambdeg;
    phi = (1-B3)*(anglisdeg+deltaSh)+(B3/B2)*atan(B2*(anglisdeg+deltaSh));
...
// -------------------
    ...
    C2  = (-1.86*pow(FradkN,2)-2.73*FradkN)/(C0*C1*exp(0.11*FradkN))*(1-0.03*fabs(angcambdeg));
    ...
    phi = (1-C3)*(anglisdeg+deltaSh)+(C3/C2)*atan(C2*(anglisdeg+deltaSh));
