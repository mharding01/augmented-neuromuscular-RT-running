...
int ModelSimuIndex::get_mbs_joint_index(int joint_id)
{
	switch (joint_id)
	{
		...
		case SimuJointIndex::RightFlexProx  : return RFlex_pr_T3_id;
		case SimuJointIndex::RightFlexDist  : return RFlex_ds_T3_id;
		...
		case SimuJointIndex::LeftFlexProx  : return LFlex_pr_T3_id;
		case SimuJointIndex::LeftFlexDist  : return LFlex_ds_T3_id;
...
int ModelSimuIndex::get_mbs_body_index(int body_id)
{
	switch (body_id)
	{
		...
		case SimuBodyIndex::RFlexPr  : return RFlexPr_id;
		case SimuBodyIndex::RFlexDs  : return RFlexDs_id;
		...
		case SimuBodyIndex::LFlexPr  : return LFlexPr_id;
		case SimuBodyIndex::LFlexDs  : return LFlexDs_id;
...
int ModelSimuIndex::get_mbs_Ssens_index(int Ssens_id)
{
	switch (Ssens_id)
	{
		...
		// upper feet part
		case SimuSsensIndex::RightFoot : return RFoot_Upper_id;
		case SimuSsensIndex::LeftFoot  : return LFoot_Upper_id;

		// one part feet
		case SimuSsensIndex::RightFlexProx : return mbs_data->Nsensor + RFlex_pr_force_id;
		case SimuSsensIndex::RightFlexDist : return mbs_data->Nsensor + RFlex_ds_force_id;
		case SimuSsensIndex::LeftFlexProx  : return mbs_data->Nsensor + LFlex_pr_force_id;
		case SimuSsensIndex::LeftFlexDist  : return mbs_data->Nsensor + LFlex_ds_force_id;
...
int ModelSimuIndex::get_mbs_Fsens_index(int Fsens_id)
{
	switch (Fsens_id)
	{
		...
		// one part feet
		case SimuFsensIndex::RightFlexProx : return RFlex_pr_force_id;
		case SimuFsensIndex::RightFlexDist : return RFlex_ds_force_id;
		case SimuFsensIndex::LeftFlexProx  : return LFlex_pr_force_id;
		case SimuFsensIndex::LeftFlexDist  : return LFlex_ds_force_id;
...
int ModelSimuIndex::get_mbs_link_index(int link_id)
{
	switch (link_id)
	{
		case SimuLinkIndex::RightFlexProx : return Link_RFlexProx_id;
		case SimuLinkIndex::RightFlexDist : return Link_RFlexDist_id;
		case SimuLinkIndex::LeftFlexProx  : return Link_LFlexProx_id;
		case SimuLinkIndex::LeftFlexDist  : return Link_LFlexDist_id;
	
		default:
			std::cout << "Error: unknown link index: " << link_id << std::endl;
			exit(EXIT_FAILURE);
	}
