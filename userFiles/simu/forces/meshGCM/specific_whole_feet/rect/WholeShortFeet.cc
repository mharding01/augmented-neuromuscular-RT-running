 ...
 */
WholeShortFeet::WholeShortFeet(MbsData *mbs_data, GaitFeatures *gait_features, SensorsInfo *sens_info, GroundModel *ground): WholeFeet(mbs_data, gait_features, sens_info, ground)
{
	for(int i=0; i<NB_FEET_BODIES; i++)
	{
		switch (i)
		{
			case RIGHT_ID : bodies.push_back(new ShortFoot(mbs_data, gait_features, sens_info, R_FOOT_ID)); break;
			case LEFT_ID  : bodies.push_back(new ShortFoot(mbs_data, gait_features, sens_info, L_FOOT_ID )); break;
