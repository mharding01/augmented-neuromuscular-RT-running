 ...
 */
WholeRectLongFeet::WholeRectLongFeet(MbsData *mbs_data, GaitFeatures *gait_features, SensorsInfo *sens_info, GroundModel *ground): WholeFeet(mbs_data, gait_features, sens_info, ground)
{
	for(int i=0; i<NB_FEET_BODIES; i++)
	{
		switch (i)
		{
			case RIGHT_ID : bodies.push_back(new RectLongFoot(mbs_data, gait_features, sens_info, R_FOOT_ID)); break;
			case LEFT_ID  : bodies.push_back(new RectLongFoot(mbs_data, gait_features, sens_info, L_FOOT_ID )); break;
...
/*! \brief destructor
 */
WholeRectLongFeet::~WholeRectLongFeet()
{
 ...
 */
void WholeRectLongFeet::compute_F_T(double F_tot[3], double T_tot[3], double PxF[4], double RxF[4][4], double VxF[4], double OMxF[4], int index)
{
	ContactFoot *body;
 ...
 */
double WholeRectLongFeet::get_leg_feet_forces(int leg_id, int axis)
{
	switch (leg_id)
	{
		case RIGHT_ID:
 ...
 */
double WholeRectLongFeet::get_leg_feet_torques(int leg_id, int axis)
{
	switch (leg_id)
	{
		case RIGHT_ID:
